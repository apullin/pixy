...
@@ <function block>
    ...
    .global lineProcessedRL0      @@ - Symbol name for function
    .code   16              @@ - 16bit THUMB code (BOTH are required!)
    .thumb_func             @@ /
    .type   lineProcessedRL0, %function   @@ - symbol type (not req)
...
@@ <function block>
	.text
    .align  2               @@ - 2^n alignment (n=2)
    .global lineProcessedRL1      @@ - Symbol name for function
    .code   16              @@ - 16bit THUMB code (BOTH are required!)
    .thumb_func             @@ /
    .type   lineProcessedRL1, %function   @@ - symbol type (not req)
...
// 2
.loop6:
		MOV		r7, r5 // copy lut val (lutPrev)
		MOV		r5, r8
		CMP		r3, r5
		BGE		dest30
		NOP
...
@@ <function block>
	.text
    .align  2               @@ - 2^n alignment (n=2)
    .global lineProcessedRL0A      @@ - Symbol name for function
    .code   16              @@ - 16bit THUMB code (BOTH are required!)
    .thumb_func             @@ /
    .type   lineProcessedRL0A, %function   @@ - symbol type (not req)
...
@@ <function block>
	.text
    .align  2               @@ - 2^n alignment (n=2)
    .global lineProcessedRL1A      @@ - Symbol name for function
    .code   16              @@ - 16bit THUMB code (BOTH are required!)
    .thumb_func             @@ /
    .type   lineProcessedRL1A, %function   @@ - symbol type (not req)
...
//		// cycle
//		MEND
#define RED		LDRB 	r6, [r0] // load red pixel 
...
//		// cycle
//		MEND
#define GREEN		LDRB 	r5, [r0] // load red pixel 
...
//		// cycle
//		MEND
#define RED		LDRB 	r6, [r0, r4] // load red pixel 
...
//		// cycle
//		MEND
#define GREEN		LDRB 	r5, [r0, r4] // load green pixel \
					ADDS	r4, #1		
...
$lx		QVAL
		...
		MEND
*/
#define QVAL		MOV 	r5, r10; // r5 gets beginning column
					LSLS 	r6, r5, #3; // r6 gets shifted beginning column
					ORRS	r6, r7; // add model
					SUBS  	r1, r4, r5; // r1 gets length
					LSLS 	r5, r1, #12; // r5 gets shifted length 
...
.wrap:
		MOVS	r3, #0    // reset qq index
		B .lcpy

.ecpy:
		LSRS    r0, #2 // return number of qvals
		POP		{r1-r7, pc}